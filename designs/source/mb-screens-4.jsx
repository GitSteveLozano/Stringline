    ...
    <div className="m">
      ...
      <div className="m-body">
        <div className="m-stat-strip">
          ...
          {/* Tappable — opens RentalsUtilization */}
          <button style={{flex:1, background:'transparent', border:'none', textAlign:'left', padding:0, fontFamily:'inherit', cursor:'pointer'}}>
            <div className="m-stat-strip-l" style={{display:'flex', alignItems:'center', gap:4}}>
              <span>Util · 30d</span>
              <span style={{color:'#aea69a'}}><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 6l6 6-6 6"/></svg></span>
            </div>
            <div className="m-stat-strip-v num" style={{color:'#2c8a55'}}>92%</div>
          </button>
        </div>
        {[
          {n:'Scaffold A — 24×8', cat:'Scaffolding', tag:'SCF-001', s:'out', proj:'Hillcrest Mews', dur:'Day 12 of 32', rate:'$85/day', draft:true},
          ...
          <div key={r.tag} style={{margin:'10px 16px 0', background:'#fff', border:'1px solid #e8e3db', borderRadius:12, overflow:'hidden'}}>
            <div style={{padding:'12px 14px', display:'flex', gap:12, alignItems:'center'}}>
              ...
              <div style={{textAlign:'right'}}>
                {r.s === 'out' && <span className="m-pill" data-tone="amber" data-dot="true">out</span>}
                {r.s === 'available' && <span className="m-pill" data-tone="green" data-dot="true">avail</span>}
                {r.s === 'service' && <span className="m-pill" data-tone="red" data-dot="true">service</span>}
                {r.draft && <div style={{fontSize:10, color:'#b46e2c', fontWeight:600, marginTop:5, display:'flex', alignItems:'center', justifyContent:'flex-end', gap:4}}>
                  <span style={{width:5, height:5, borderRadius:3, background:'#d9904a'}}/>draft return
                </div>}
  ...
  return (
    <div className="m">
      ...
      <div className="m-body">
        ...
        {/* Setup checklist — only present until all 4 ambient tasks are done */}
        <div style={{padding:'4px 16px 0'}}>
          <div style={{padding:'12px 14px', background:'#fff', border:'1px solid #e8e3db', borderLeft:'3px solid #d9904a', borderRadius:11, display:'flex', alignItems:'center', gap:12}}>
            {/* Progress ring */}
            <div style={{width:36, height:36, position:'relative', flexShrink:0}}>
              <svg width="36" height="36" viewBox="0 0 36 36">
                <circle cx="18" cy="18" r="15" fill="none" stroke="#f5f1ec" strokeWidth="3"/>
                <circle cx="18" cy="18" r="15" fill="none" stroke="#d9904a" strokeWidth="3" strokeDasharray="94 94" strokeDashoffset="62" transform="rotate(-90 18 18)" strokeLinecap="round"/>
              </svg>
              <div style={{position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:700, fontFeatureSettings:'"tnum"'}}>2/6</div>
            </div>
            <div style={{flex:1}}>
              <div style={{fontSize:13, fontWeight:600}}>Finish workspace setup</div>
              <div style={{fontSize:11, color:'#8a8278'}}>4 ambient tasks · ~6 min total</div>
            </div>
            <span className="m-chev" style={{color:'#aea69a'}}>{MI.chev}</span>
          </div>
        </div>
        ...
        <div className="m-list-inset">
          ...
          <MRow leading={MI.$} leadingTone="green" headline="Loaded labor cost" supporting="$54.20 / hr · base + add-ons" trailing="→ Time"/>
          <MRow leading={MI.cal} headline="Working hours + holidays" supporting="Mon–Sat 7–4 · 8 holidays"/>
          <MRow leading={MI.receipt} headline="Estimates + invoices" supporting="Cover note, nudges, PDF protection"/>
        </div>
        ...
        <div className="m-list-inset">
          <MRow leading={MI.user} headline="Profile" supporting="Name, contact, devices"/>
          ...
          <MRow leading={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></svg>} headline="Help + support" supporting="Chat · book a call · changelog"/>
        </div>
...
function SettingsIntegRow({ logo, tone, name, status, connected, sync }) {
  ...
  return (
    <div style={{padding:'12px 14px', borderBottom:'1px solid #e8e3db', display:'flex', alignItems:'center', gap:12}}>
      ...
      {connected
        ? <span className="m-pill" data-tone="green" data-dot="true">connected</span>
...
// ============================================================
// SettingsHours — working days + hours + holidays (edit context)
// Wired from SettingsHome → "Working hours + holidays" row.
// ============================================================
function SettingsHours() {
  ...
  const holidays = [
    { d:'Jan 1',  l:'New Year\u2019s Day' },
    { d:'May 19', l:'Victoria Day' },
    { d:'Jul 1',  l:'Canada Day' },
    { d:'Sep 1',  l:'Labour Day' },
    { d:'Oct 13', l:'Thanksgiving' },
    { d:'Dec 25', l:'Christmas Day' },
  ];
  return (
    <div className="m">
      <MTopBar back title="Working hours" sub="& holidays"/>
      <div className="m-body">

        {/* Timezone */}
        <div className="m-section-h">Timezone</div>
        <div className="m-list-inset">
          <div style={{padding:'12px 14px', display:'flex', alignItems:'center', gap:12}}>
            <div style={{width:34, height:34, borderRadius:8, background:'rgba(217,144,74,.10)', color:'#d9904a', display:'flex', alignItems:'center', justifyContent:'center'}}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="12" cy="12" r="9"/><path d="M12 3v18M3 12h18M5 6c2 2 4 3 7 3s5-1 7-3M5 18c2-2 4-3 7-3s5 1 7 3"/></svg>
            </div>
            <div style={{flex:1}}>
              <div style={{fontSize:14, fontWeight:500}}>Mountain Time \u00b7 UTC\u22127</div>
              <div style={{fontSize:11, color:'#8a8278'}}>From device \u00b7 used for clock-in</div>
            </div>
            <span className="m-chev" style={{color:'#aea69a'}}>{MI.chev}</span>
          </div>
        </div>
        ...
        {/* Working days */}
        <div className="m-section-h">Working days</div>
        <div style={{padding:'0 16px'}}>
          <div style={{padding:'12px', background:'#fff', border:'1px solid #e8e3db', borderRadius:12}}>
            <div style={{display:'flex', gap:6}}>
              {days.map((d, i) => (
                <div key={i} style={{
                  flex:1, height:40, borderRadius:10,
                  background: d.on ? '#d9904a' : '#fff',
                  border: d.on ? '1.5px solid #d9904a' : '1.5px solid #e8e3db',
                  color: d.on ? '#fff' : '#8a8278',
            ...
            <div style={{display:'flex', justifyContent:'space-between', marginTop:14, gap:10}}>
              <div style={{flex:1, padding:'10px 14px', background:'#f7f4ef', borderRadius:10}}>
                <div style={{fontSize:10, color:'#8a8278', textTransform:'uppercase', letterSpacing:'.06em', fontWeight:600}}>Start</div>
                <div style={{fontSize:18, fontWeight:600, fontFeatureSettings:'"tnum"', marginTop:2}}>7:00 <span style={{fontSize:12, color:'#8a8278', fontWeight:500}}>AM</span></div>
              </div>
              <div style={{flex:1, padding:'10px 14px', background:'#f7f4ef', borderRadius:10}}>
                <div style={{fontSize:10, color:'#8a8278', textTransform:'uppercase', letterSpacing:'.06em', fontWeight:600}}>End</div>
                <div style={{fontSize:18, fontWeight:600, fontFeatureSettings:'"tnum"', marginTop:2}}>5:00 <span style={{fontSize:12, color:'#8a8278', fontWeight:500}}>PM</span></div>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Overtime + breaks */}
        <div className="m-section-h">Rules</div>
        <div className="m-list-inset">
          <MRow leading={MI.time} headline="Overtime" supporting="After 40h / week" trailing="1.5x"/>
          <MRow leading={MI.layers} headline="Auto-deduct break" supporting="30 min after 6 hours" trailing={<span className="m-pill" data-tone="accent">on</span>}/>
          <MRow leading={MI.alert} leadingTone="amber" headline="Grace period" supporting="Allowed clock-in window" trailing="\u00b110 min"/>
        </div>

        {/* Holidays */}
        <div className="m-section-h-row">
          <div className="m-section-h" style={{flex:1, padding:'18px 0 8px 0'}}>Holidays (8)</div>
          <button style={{background:'transparent', border:'none', color:'#d9904a', fontFamily:'inherit', fontSize:13, fontWeight:500, padding:'18px 20px 8px 0'}}>+ Add</button>
        </div>
        <div className="m-list-inset">
          {holidays.map(h => (
            <div key={h.d} style={{padding:'10px 14px', display:'flex', alignItems:'center', gap:12, borderBottom:'1px solid #e8e3db'}}>
              <div style={{width:42, fontSize:11, color:'#8a8278', fontWeight:600, fontFeatureSettings:'"tnum"'}}>{h.d}</div>
              <div style={{flex:1, fontSize:13, fontWeight:500}}>{h.l}</div>
              <span style={{fontSize:11, color:'#8a8278'}}>paid</span>
            </div>
          ))}
        </div>

        <div className="m-btn-stack" style={{margin:'18px 0 24px'}}>
          <button className="m-btn" data-variant="primary">Save changes</button>
        </div>
      </div>
    </div>
  );
...
// Worker bottom tabs — duplicated here (also in mb-screens-5) so both files
// can render their own worker screens without cross-file scope issues.
// Both definitions must stay in sync.
function WorkerTabsLocal({ active = 'today' }) {
  const tabs = [
    {l:'Today', id:'today', i:MI.home},
    {l:'Scope', id:'scope', i:MI.list},
    {l:'Hours', id:'hours', i:MI.time},
    {l:'Log',   id:'log',   i:MI.cam},
  ];
  return (
    <>
      <div style={{height:64, flexShrink:0}}/>
      <div style={{position:'absolute', bottom:0, left:0, right:0, display:'flex', height:64, background:'#0e0c0a', borderTop:'1px solid #2a241c', zIndex:5}}>
        {tabs.map(t => {
          const on = t.id === active;
          return (
            <div key={t.id} style={{flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:3, color: on ? '#d9904a' : '#5a5346'}}>
              {t.i}
